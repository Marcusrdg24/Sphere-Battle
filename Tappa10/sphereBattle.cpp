...
struct Sphere {
    ...
    void attack(Sphere& target, unsigned& schermata, sf::RenderWindow& window) {
        if(dati.name == "Boxer") {
            ...
            // Movimento nuvola
            if (!dati.can_draw && dati.cooldown >= cooldown_magic) {
            ...
            // Attacco            
            if (!dati.can_draw && dati.cooldown > cooldown2_magic &&
            ...
            // Reset attacco
            if (dati.can_draw && dati.cooldown >= max_frame_rate) {
                dati.can_draw = false;
                dati.cooldown = 0;
                dati.hit_self = false;
                cloudPos = sf::Vector2f (   std::rand() % static_cast<int>(right_wall - left_wall - weapon.getSize().x) + left_wall + (weapon.getSize().x / 2.f), 
            ...
            // Posizione trappola
            if (!dati.can_draw && dati.cooldown > cooldown_hunter) {
            ...
            if (dati.can_draw) {
                ...
                if (dati.cooldown > cooldown2_hunter) {
                    dati.can_draw = false;
                    dati.cooldown = cooldown_hunter;
                }
            }
