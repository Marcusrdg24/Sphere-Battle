...
const int cooldown2_magic = max_frame_rate;
const int cooldown_hunter = max_frame_rate;
const int cooldown2_hunter = max_frame_rate * 6;
...
struct Cooldown {
    ...
    Cooldown(bool isP1) {
        ...
        // Stile
        cooldownBar.setFillColor(sf::Color(180, 180, 180));
        
        // Posizione
        if (isP1) {
            cooldownBar.setSize(sf::Vector2f(125.f, 30.f));
            cooldownBar.setOrigin({0.f, 15.f});
            cooldownBar.setPosition({sf::Vector2f(244.f - thickness, window_height - 120.f)});
        }
        else {
            cooldownBar.setSize(sf::Vector2f(125.f, 30.f));
            cooldownBar.setOrigin({125.f, 15.f});
            cooldownBar.setPosition({sf::Vector2f(window_width - 244.f + thickness, window_height - 120.f)});
        }
    }

    // Metodi
    void modifyCooldown (const int maxCooldown) {
        float currentWidth = 125.f;
        if (cooldown < maxCooldown) {
            currentWidth = 125.f * cooldown / maxCooldown;
        }
        cooldownBar.setSize(sf::Vector2f(currentWidth, 30.f));
        if (isP1) {
            cooldownBar.setOrigin(sf::Vector2f(0.f, 15.f));
        }
        else {
            cooldownBar.setOrigin(sf::Vector2f(currentWidth, 15.f));
        }
    }
};
...
SphereData magic (  "Magic", sf::Color(67, 0, 255), "../Utilities/Images/Blue_Thunder.png", "../Utilities/Weapons/Blue_Cloud.png",
                    "../Utilities/Weapons/Blue_Thunder.png", "Mago del clima", "Fulmini", 120); // Arma e proiettile

SphereData hunter ( "Hunter", sf::Color(252, 0, 255), "../Utilities/Images/Purple_Trap.png", 
                    "../Utilities/Weapons/Purple_Trap.png", "Cacciatore spietato", "Trappole", 130, false); // Proiettile
...
struct Sphere {
    ...
    bool sphere_loader(float radius, SphereData dati) {
        ...
        if (dati.weaponPath != "") {
            ...
            weaponSprite.setPosition(sf::Vector2f(0.f, 0.f));
        }
        ...
        if (dati.bulletPath != "") {
            ...
            bulletSprite.setRotation(sf::degrees(0));
            ...
            bulletSprite.setPosition(sf::Vector2f(0.f, 0.f));
        }
    ...
    void attack(Sphere& target, unsigned& schermata, sf::RenderWindow& window) {
        if(dati.name == "Boxer") {
            ...
            // Scontro
            if (dati.cooldown.cooldown < cooldown_boxer) {
                dati.cooldown.cooldown++;
            }
                ...
                if (dotProduct > 0.f) {
                    ...
                    if (target.dati.name == "Boxer") {
                        ...
                        dati.cooldown.cooldown = 0;
                        target.dati.cooldown.cooldown = 0;
                    }
                }
            }

            // Modifica barra del cooldown
            dati.cooldown.modifyCooldown(cooldown_boxer);
            ...
            // Cooldown
            if (dati.cooldown.cooldown < cooldown_cowboy) {
                dati.cooldown.cooldown++;
            }
            ...
            // Modifica barra del cooldown
            dati.cooldown.modifyCooldown(cooldown_cowboy);
            ...
            // Cooldown
            if (dati.cooldown.cooldown < cooldown_chef && !dati.can_draw) {
                dati.cooldown.cooldown++;
            }
            ...
            // Modifica barra del cooldown
            dati.cooldown.modifyCooldown(cooldown_chef);
            ...
            // Cooldown
            if (dati.cooldown.cooldown < cooldown_killer && !dati.can_draw) {
                dati.cooldown.cooldown++;
            }
            ...
            // Modifica barra del cooldown
            dati.cooldown.modifyCooldown(cooldown_killer);
        ...
        else if (dati.name == "Magic") {
            // Vettori di posizione
            sf::Vector2f pos = ball.getPosition();
            sf::Vector2f targetPos = target.ball.getPosition();
            sf::Vector2f cloudPos = weaponSprite.getPosition();

            // Posizionameto iniziale
            if (weaponSprite.getPosition() == sf::Vector2f(0.f, 0.f)) {
                dati.cooldown.cooldown = cooldown_magic;
            }
            ...
            // Movimento nuvola
            if (!dati.can_draw && dati.cooldown.cooldown >= cooldown_magic) {
                cloudPos = sf::Vector2f (   std::rand() % static_cast<int>(right_wall - left_wall - weapon.getSize().x) + left_wall + (weapon.getSize().x / 2.f), 
                                            150.f + (weapon.getSize().y / 2.f));
                weaponSprite.setPosition(cloudPos);
                bulletSprite.setPosition(sf::Vector2f(cloudPos.x, cloudPos.y + (weapon.getSize().y / 2.f) + (bullet.getSize().y / 2.f)));
                dati.cooldown.cooldown = 0;
            }

            // Attacco            
            if (!dati.can_draw && dati.cooldown.cooldown > cooldown2_magic &&
                targetPos.x >= cloudPos.x - (weapon.getSize().x / 2.f) &&
                targetPos.x <= cloudPos.x + (weapon.getSize().x / 2.f)) {
                    dati.can_draw = true;
                    dati.cooldown.cooldown = 0;
                    target.health.modifyHealth(dati.strength, schermata);
            ...
            // Attacco self
            if (dati.can_draw && !dati.hit_self && 
                pos.x + (radius_sphere_simulation / 2.f) >= cloudPos.x - (weapon.getSize().x / 2.f) &&
                pos.x - (radius_sphere_simulation / 2.f) <= cloudPos.x + (weapon.getSize().x / 2.f)) {
                    dati.hit_self = true;
                    health.modifyHealth(dati.strength / 2, schermata);     
            }

            // Reset attacco
            if (dati.can_draw && dati.cooldown.cooldown >= max_frame_rate) {
                dati.can_draw = false;
                dati.cooldown.cooldown = 0;
                dati.hit_self = false;
                cloudPos = sf::Vector2f (   std::rand() % static_cast<int>(right_wall - left_wall - weapon.getSize().x) + left_wall + (weapon.getSize().x / 2.f), 
            ...
            // Modifica barra del cooldown
            if (!dati.can_draw) {
                dati.cooldown.modifyCooldown(cooldown2_magic);
            }
            else {
                dati.cooldown.cooldownBar.setSize(sf::Vector2f(0.f, 30.f));
            }
        ...
        else if (dati.name == "Hunter") {
            // Posizione trappola
            if (!dati.can_draw && dati.cooldown.cooldown > cooldown_hunter) {
                bulletSprite.setPosition(ball.getPosition());
                dati.can_draw = true;
                dati.cooldown.cooldown = 0;
            }
            ...
            if (dati.can_draw) {
                // Variabili
                sf::Vector2f targetPos = target.ball.getPosition();
                sf::Vector2f trapPos = bulletSprite.getPosition();

                // Controllo collisione sfera e trappola (Generato da Gemini e riadattato per l'attacco dell'hunter)
                // Distanze
                sf::Vector2f delta = targetPos - trapPos;
                float distance = std::sqrt(delta.x * delta.x + delta.y * delta.y);
                float minDistance = radius_sphere_simulation + bullet.getSize().x / 2.f;
                ...
                if (dati.cooldown.cooldown > cooldown2_hunter) {
                    dati.can_draw = false;
                    dati.cooldown.cooldown = cooldown_hunter;
                }
            }

            // Modifica barra del cooldown
            if (!dati.can_draw) {
                dati.cooldown.modifyCooldown(cooldown_hunter);
            }
            else {
                dati.cooldown.cooldownBar.setSize(sf::Vector2f(0.f, 30.f));
            }
...
struct State {
    ...
    void moveGame() {
        ...
        ball1.move();
        ball2.move();
        ball1.attack(ball2, schermata, window);
        ball2.attack(ball1, schermata, window);
    }
...
void initialize_arena (State& stato, std::vector<sf::Text>& balls_name, std::vector<sf::Text>& balls_health) {
    ...
    stato.ball1.dati.cooldown = Cooldown(true);
    ...
    stato.ball2.dati.cooldown = Cooldown(false);

    // Ripristino cooldown
    stato.ball1.dati.cooldown.cooldown = 0;
    stato.ball2.dati.cooldown.cooldown = 0;
...
void drawBattleSimulation(  State& stato, sf::RectangleShape arena, sf::RectangleShape healthContainer1, sf::RectangleShape healthContainer2,
                            sf::RectangleShape cooldownContainer1, sf::RectangleShape cooldownContainer2, std::vector<sf::Text> balls_name, std::vector<sf::Text> balls_health) {
    ...
    stato.window.draw(cooldownContainer1);
    stato.window.draw(cooldownContainer2);
    stato.window.draw(stato.ball1.dati.cooldown.cooldownBar);
    stato.window.draw(stato.ball2.dati.cooldown.cooldownBar);
...
sf::RectangleShape BS_healthContainer_loader(bool isP1) {
    ...
    // Posizione
    sf::FloatRect contBounds = healthContainer.getLocalBounds();
    healthContainer.setOrigin({ contBounds.position.x + contBounds.size.x / 2.f, 
                                contBounds.position.y + contBounds.size.y / 2.f});
    if (isP1) {
...
sf::RectangleShape BS_cooldownContainer_loader(bool isP1) {
    sf::RectangleShape cooldownContainer;

    // Stile
    cooldownContainer.setSize(sf::Vector2f(125.f, 30.f));
    cooldownContainer.setFillColor(sf::Color::Transparent);
    cooldownContainer.setOutlineColor(sf::Color::Black);
    cooldownContainer.setOutlineThickness(-thickness);
    
    // Posizione
    sf::FloatRect contBounds = cooldownContainer.getLocalBounds();
    cooldownContainer.setOrigin({   contBounds.position.x + contBounds.size.x / 2.f, 
                                    contBounds.position.y + contBounds.size.y / 2.f});
    if (isP1) {
        cooldownContainer.setPosition({sf::Vector2f(244.f + (contBounds.size.x / 2.f), window_height - 120.f)});
    }
    else {
        cooldownContainer.setPosition({sf::Vector2f(window_width - 244.f - (contBounds.size.x / 2.f), window_height - 120.f)});
    }
...
int main () {
    ...
    sf::RectangleShape BS_cooldownContainer1 = BS_cooldownContainer_loader(true);
    sf::RectangleShape BS_cooldownContainer2 = BS_cooldownContainer_loader(false);
    ...
    while (stato.window.isOpen()) {
        ...
        if (stato.schermata == 0) {
            ...
            stato.window.clear(sf::Color(0, 255, 255));
            drawBattleSimulation(stato, BS_arena, BS_healthContainer1, BS_healthContainer2, BS_cooldownContainer1, BS_cooldownContainer2, BS_ballsName, BS_ballsHealth);
